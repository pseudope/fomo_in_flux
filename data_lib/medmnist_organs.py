...
BASE_CLASSES = json.load(open("data_lib/00_info/MedMNISTorgans_classnames.json", "r"))
...
PARAMS = {
    ...
    "mean": (0.4950, 0.4950, 0.4950),
    "std": (0.2828, 0.2828, 0.2828),
    # Default Imagesize
    "img_size": 28,
...
class Dataset(data_lib.DatasetScaffold):
    def __init__(self, root, **kwargs) -> None:
        ...
        self.root = os.path.join(root, "MedMNISTorgans")
        self.PARAMS = PARAMS
    ...
    def _download_and_setup(self):
        ...
        image_folder = f"{self.root}/medmnistorgans-images-{self.split}"
        if not os.path.exists(image_folder):
            self.medmnist_organc_dataset = medmnist.OrganSMNIST(
            ...
            for i in tqdm.trange(len(data), desc="Creating MedMNISTorgans images..."):
