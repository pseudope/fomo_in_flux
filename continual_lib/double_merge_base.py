...
class DoubleMergeBase(continual_lib.BaseContinualLearner):
    """
    ...
    def end_task(self, experiment, **kwargs):
        ...
        with torch.no_grad():
            ...
            for mode in ["backbone", "head"]:
                ...
                self.checkpoint_storage["running"][mode].append(
                    copy.deepcopy(base_state_dicts[mode])
                )
    ...
    def unrolled_merge(self, checkpoint_list, mode):
        running_checkpoint = copy.deepcopy(checkpoint_list[0])
        for checkpoint in checkpoint_list[1:]:
            running_checkpoint = copy.deepcopy(
                self.f_merge[mode](
                    [copy.deepcopy(checkpoint), running_checkpoint],
                    zero_shot=copy.deepcopy(self.checkpoint_storage["init"][mode]),
                )
            )
        return running_checkpoint

    def all_merge(self, checkpoints, mode):
        checkpoints = [copy.deepcopy(x) for x in checkpoints]
        return copy.deepcopy(
            self.f_merge[mode](checkpoints, zero_shot=copy.deepcopy(self.checkpoint_storage["init"][mode]))
        )

    def define_evaluation_weights(self, **kwargs):
        raise NotImplementedError(
            "All-merge evaluation not implemented for this class."
        )

    def define_training_weights(self, **kwargs):
        raise NotImplementedError(
            "Latest checkpoint for training not implemented for this class."
        )
